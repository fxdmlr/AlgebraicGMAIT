...
class rexp_poly:
    ...
    def extrema(self):
        ...
        for i in arr:
            if abs(i.imag) < 10**(-6):
                new_arr.append([i.real, sgn(self.diff().diff()(i.real))])
        return new_arr
    ...
    def integrate(self):
        return ratexp_integration_real(self.p1, self.p2)
...
class sqrt:
    ...
    def diff(self, wrt=0):
        return Comp([Prod([2, sqrt()]), rexp_poly(1, poly([0, 1]))])
    __rmul__ = __mul__
    __radd__ = __add__
...
class cbrt:
    ...
    def diff(self, wrt=0):
        return Comp([Prod([3, Comp([poly([0, 0, 1]), cbrt()])]), rexp_poly(1, poly([0, 1]))])
    __rmul__ = __mul__
    __radd__ = __add__
...
class asin:
    ...
    def diff(self, wrt=0):
        return Comp([poly([1, 0, -1]), sqrt(), rexp_poly(1, poly([0, 1]))])
    __rmul__ = __mul__
    __radd__ = __add__
...
class sinc:
    def __init__(self):
        self.function = lambda x : 1 if x == 0 else cmath.sin(math.pi * x) / (math.pi * x)
        self.user_args = dict([(key,val) for key,val in locals().items() if key!='self' and key!='__class__'])
    ...
    def npprint(self, prev_ppr=[[" "], [" "], [" "], ["x"], [" "], [" "], [" "]]):
        ...
        return connect(s[:], connect(op[:], connect(prev_ppr[:], clsd[:])))[:]
    def texify(self, prev_tex="x"):
        return 'sinc(%s)'%prev_tex[:]
    def diff(self, wrt=0):
        return Div([poly([0, math.pi**2 ]) * Comp([poly([0, math.pi]), cos()]) - math.pi * Comp([poly([0, math.pi]), sin()]), poly([0, 0, math.pi**2])])
    __rmul__ = __mul__
    __radd__ = __add__
class diracDelta:
    def __init__(self):
        self.function = lambda x : float("inf") if x == 0 else 0
        self.user_args = dict([(key,val) for key,val in locals().items() if key!='self' and key!='__class__'])
    ...
    def __mul__(self, other):
        if callable(other):
            return Prod([other(0), diracDelta()])
        else:
            return Prod([other, diracDelta()])
    def __neg__(self):
        return Prod([-1, self])
    def __pow__(self, n):
        i = self
    ...
    def npprint(self, prev_ppr=[[" "], [" "], [" "], ["x"], [" "], [" "], [" "]]):
        ...
        if len(prev_ppr[0]) == 1:
            op = [[" "], [" "], [" "], [" "], [" "], [" "], [" "]]
            clsd = [[" "], [" "], [" "], [" "], [" "], [" "], [" "]]
        s = npprify("diracDelta")
        return connect(s[:], connect(op[:], connect(prev_ppr[:], clsd[:])))[:]
    def texify(self, prev_tex="x"):
        return 'diracDelta(%s)'%prev_tex[:]
    def diff(self, wrt=0):
        return Comp([poly([+0.001]), diracDelta()])*0.5+ Comp([poly([-0.001]), diracDelta()])*(-0.5)
    __rmul__ = __mul__
    __radd__ = __add__
...
def integrate_poly_ln(p, q):
    # integrates p(x)ln(q(x))
    # where p(x) and q are polys
    return Prod([p.integrate(), Comp([q, log()])]) + rexp_poly(-p.integrate() * q.diff(), q).integrate()

def integrate_poly_atan(p, q):
    return Prod([p.integrate(), Comp([q, atan()])]) + rexp_poly(-p.integrate()*q.diff(), q**2 + 1).integrate()