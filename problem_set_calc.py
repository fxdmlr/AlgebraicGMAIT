...
def tgline(objects):
    x = poly([0, 1])
    f = objects[0].diff()(objects[1]) * x - objects[0].diff()(objects[1]) * objects[1] + objects[0](objects[1])
    return f(objects[2])

def extremasolve(objects):
    ext = objects[0].extrema()
    z = abs(ext[0][0] - objects[1])
    ind = ext[0][0]
    for i in ext:
        if abs(i[0] - objects[1]) < z:
            z = abs(i[0] - objects[1])
            ind = i[0]
    
    return objects[0](ind)

def ndiffgame(objects):
    a = objects[1]
...
number_generators = [
    ['the line tangent to $ at x = $ evaluated at x = $ ', tgline, [rexp_poly.rand, rand_num, rand_num], [1, 0, 0]], 
    ['$th derivative of $ at x = $ ', lambda objects : objects[1].ndiff(objects[0])(objects[2]), [rand_num, rexp_poly.rand, rand_num], [1, 0, 0]],
    ['the value of $ at its nearest extrema to x = $ ', extremasolve, [rexp_poly.rand, rand_num], [1, 0]],
    ['the line tangent to $ at x = $ evaluated at x = $ ', tgline, [rand_func_iii, rand_num, rand_num], [1, 0, 0]],
    ['second derivative of $ at x = $ ', lambda objects: (objects[0].diff()).diff()(objects[1]), [rand_func_iii, rand_num], [1, 0]],
    ['third derivative of $ at x = $ ', lambda objects: ((objects[0].diff()).diff()).diff()(objects[1]), [rand_func_iii, rand_num], [1, 0]],
