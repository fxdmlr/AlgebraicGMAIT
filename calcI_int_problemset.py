...
def solve_poly_trig_e(p, n, a, b, t=0):
    r, s = solve_poly_trig(p, n, t=t)
    f = lambda x : r(x) * cmath.exp(x * n * complex(0, 1)) + s(x) * cmath.exp(-x * n * complex(0, 1))
    return f(b) - f(a)


def solve_poly_exp_e(p, n, a, b, t=0):
    r = solve_poly_exp(p, n)
...
def cfseries_poly(objects):
    f = complex_fourier_series_poly(objects[4], objects[5], objects[6], objects[7])
...
def int_type1(objects):
    ...
    f = ratexp_integration_real(objects[0].arr[0], p)
...
def int_euler(objects):
    p, q, r = objects[0].arr
    f = integrate_euler(p, q, r)
    return f(objects[2]) - f(objects[1])

def int_quad_p(objects):
    p, r, n = objects[0].arr 
    f = integrate_q_p_n(p, r, n)
    return f(objects[2]) - f(objects[1])

def int_trig_rat(objects):
    p, q = objects[0].arr 
    f = integrate_trig_rat(p, q)
    return f(objects[2]) - f(objects[1])
...
number_generators = [
    ...
    ['the integral of f(x) = $ from $ to $', lambda objects : objects[0].integrate()(objects[2]) - objects[0].integrate()(objects[1]), [rand_rat_expr, rand_num, rand_num], [5, 0, 0]],
    ['the integral of f(x) = ($)exp($x) from $ to $ ', lambda objects : solve_poly_exp_e(objects[0], objects[1], objects[2], objects[3]), [rand_poly, rand_num, rand_num, rand_num], [1, 0, 0, 0]],
    ['the integral of f(x) = ($)cos($x) from $ to $ ', lambda objects : (solve_poly_trig_e(objects[0], objects[1], objects[2], objects[3], t=1)).real, [rand_poly, rand_num, rand_num, rand_num], [1, 0, 0, 0]],
    ['the integral of f(x) = ($)sin($x) from $ to $ ', lambda objects : (solve_poly_trig_e(objects[0], objects[1], objects[2], objects[3])).real, [rand_poly, rand_num, rand_num, rand_num], [1, 0, 0, 0]],
    ['the integral of f(x) = ($)log($) from $ to $ ', lambda objects : integrate_poly_ln(objects[0], objects[1])(objects[3]) - integrate_poly_ln(objects[0], objects[1])(objects[2]), [rand_poly, rand_poly, rand_num, rand_num], [1, 1, 0, 0]],
    ['the integral of f(x) = ($)atan($) from $ to $ ', lambda objects : integrate_poly_atan(objects[0], objects[1])(objects[3]) - integrate_poly_atan(objects[0], objects[1])(objects[2]), [rand_poly, rand_poly, rand_num, rand_num], [1, 1, 0, 0]],
